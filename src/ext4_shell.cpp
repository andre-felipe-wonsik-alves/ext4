    ...
    : curr_inode(2), curr_path("/"), img_path(img_path)
{
  ...
  commandsMap.emplace(
      "commands",
      [this](const std::vector<std::string> &) -> void
      { commands(); });

  commandsMap.emplace(
      "info", [this](const std::vector<std::string> &) -> void
      { info(); });
...
void Ext4Shell::cat(const std::string &path)
{
  ...
  std::vector<char> content = fs.read_inode_content(target_inode, target_inode_num);
